...
$(createRoomBtn).click(function () {
    ...
    connection.invoke("CreateRoom", name).catch(function (err) {
        return console.error("Room creation error: "+err.toString());
    });
});
...
function grabWebCamVideo() {
    ...
    const width = parseInt(document.getElementById('camera_width').value) || 1280;
    const height = parseInt(document.getElementById('camera_height').value) || 720;
...
connection.on("ReceiveTextMessage", (user, message) => {
...
sendButton.addEventListener("click", () => {
    ...
    if (userName && message) {
        connection.invoke("SendTextMessage", userName, message).catch(err => console.error(err.toString()));
        messageInput.value = "";
    }
});
//---------------------------------------------------------------------------chat app end
